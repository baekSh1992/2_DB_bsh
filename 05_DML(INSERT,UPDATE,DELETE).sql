...
VALUES('900', '장채현', '920808-1234567', 'b_sh@kh.or.kr',
				'01020286003', 'D1', 'J7', 'S3', 4300000, 0.2, 200,
				SYSDATE, NULL, 'N');
...
-- 선택안된 컬럼은 값이 NULL이 들어감 (DEFAULT존재 시 DEFAULT 값으로 삽입됨)
...
DELETE  FROM EMPLOYEE2 -- DELETE = SELECT와 비슷
WHERE EMP_NAME = '장채현'; --> 조회 결과가 없다. 삭제됨
...
-- 5. TRUNCATE (DDL 입니다! DML 아닙니다!) -- TRUNC 는 절삭 함수
-- 테이블의 전체 행을 삭제하는 DDL
...
-- DELETE 		  : 휴지통 버리기 ( 임시로 버림 - 롤백 가능) /  조건을 선택해서 일부 행만 삭제 가능
-- TRUNCATE : 완전히 삭제 ( 롤백 안됨 -롤백 불가능 ) / 조건 선택 불가하며 전체 행 노빠꾸 삭제