...
FROM EMPLOYEE
GROUP BY DEPT_CODE; -- > DEPT_CODE가 같은 행끼리 한 그룹이 된다.
...
SELECT DECODE( SUBSTR(EMP_NO, 8, 1), '1', '남', '2' , '여' )  성별, 
		...
		SUM(SALARY) "급여 합"
FROM EMPLOYEE
GROUP BY DECODE( SUBSTR(EMP_NO, 8, 1), '1', '남', '2' , '여' ) -- 별칭 사용X (SEL절 마지막 해석)
...
SELECT DEPT_CODE 부서코드,JOB_CODE 직급코드,COUNT(*)합계
FROM EMPLOYEE 
...
- 여러 SELECT의 결과(RESULT SET)들을 하나의 결과로 만드는 연산자

- UNION            (합집합) : 두 SELECT의 결과를 하나로 합침 (중복 제외)
- INTERSECT     (교집합) : 두 SELECT 결과의 중복되는 부분만 조회
- UNION ALL (자주사용) : UNION + INTERSECT ( 합집합에서 중복제거를 하지 않은 결과 > 전체 조회)
- MINUS				(차집합) : A에서 A,B 교집합 부분을 제거후 조회 
*/
...
-- 조회하는 컬럼의 타입, 갯수가 모두 동일해야 한다. 


-- 서로 다른 테이블이지만 컬럼의 타입, 갯수만 일치하면 집합 연산자 사용 가능!!!!
