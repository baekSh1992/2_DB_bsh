...
--    컬럼 자체에 NULL 허용/비허용을 제어하는 성질변경의 형태로 인식됨.
...
-- "KH_BSH"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
-- D1 ~ D9까지는 2자리인데 D10은 3자리수라서 추가 불가 

-- DEPT_ID 컬럼 수정
ALTER TABLE DEPT_COPY MODIFY DEPT_ID VARCHAR2(3);
--> 수정 후 다시 위에 INSERT 실행 -> 성공
...
--> 기본값을 변경했다고 해서 기존데이터가 변하지 않고, 이후 등록되는 컬럼부터 반영

-- 기존 LNAME '한국' -> 'KOREA'로 변경 / UPDATE 사용
...
-- DEPT_COPY 테이블에 PK 추가(컬럼: DEPT_ID , 제약조건명 : D_COPT_PK)
...
SELECT * FROM DEPT_COPY; -- 변경 확인 불가
SELECT * FROM D_COPY; -- 확인 가능
...
-- ORA-02291: 무결성 제약조건(KH_BSH.SYS_C008413)이 위배되었습니다- 부모 키가 없습니다
-- 부모에게 4번째 줄이 없어서 자식도 4번째 줄 생성 불가


-- TB1 삭제
DROP TABLE TB1;
-- ORA-02449: 외래 키에 의해 참조되는 고유/기본 키가 테이블에 있습니다 < 자식이 쓰고있어서 삭제 안돼 돌아가
--> 해결방법 (3 가지)
-- 1) 자식, 부모 테이블 순서로 삭제
...
/*  DDL (데이터 정의 언어) 주의 사항  2 가지  */
-- 1) DDL은 COMMIT / ROLLBACK 불가 
	--> 수정ALTER / 삭제DROP을 신중히 진행해야함.

-- 2) DDL과 DML 구문을 섞어서 사용 불가
--> DDL은 수행 시 존재하고 있는 트랜잭션을 모두 DB에 강제 COMMIT 시킴
--> DDL이 종료된 후 DML을 수행 할 수 있도록 권장.
SELECT  * FROM TB2;
COMMIT;
