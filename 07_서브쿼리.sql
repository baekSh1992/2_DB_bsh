...
-- 선택함수 사용!
--> DECODE(컬럼명, 값1, 1인경우, 값2, 2인경우... 일치하지 않는 경우) -- 부적절
...
-- > ALL, < ALL : 여러개의 결과값의 모든 값보다 큰 / 작은 경우
--                     가장 큰 값 보다 크냐? / 가장 작은 값 보다 작냐? -- 많이 쓰지는 않는다
SELECT EMP_ID, EMP_NAME,  JOB_NAME, SALARY
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME= '과장'
...
WHERE (DEPT_CODE, JOB_CODE) = (SELECT DEPT_CODE, JOB_CODE
																	FROM EMPLOYEE
																	WHERE EMP_NAME = '노옹철')
AND EMP_NAME != '노옹철';	-- 노옹철 제외하는 구문 < 이걸 생각을 못함														
...
-- 메인 쿼리															
SELECT EMP_NAME, JOB_CODE, SALARY -- 메인 쿼리
FROM EMPLOYEE
WHERE SALARY > (SELECT AVG(SALARY) 
									FROM EMPLOYEE SUB
...
SELECT E.EMP_NAME, E.JOB_CODE, J.JOB_CODE, J.JOB_NAME
FROM EMPLOYEE E
LEFT  JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
...
-- 7. 인라인 뷰(INLINE-VIEW) -- 인라인(내부에 작성되어 있는) 뷰(가상의 테이블)
