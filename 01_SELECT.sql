...
-- < 컬럼 값 산술 연산 > 
-- 컬럼 값 : 테이블 내 한 칸(==엑셀에서 한 셀)에 작성된 값(DATA)
...
-- 현재 시간만 조회하기
SELECT SYSDATE FROM DUAL; (현재시간만 조회됨)

-- DUAL(DU mmyt A b L e) 테이블 : 가짜 테이블(임시 조회용 테이블)
...
/*
 * 컬럼명 AS 별칭 : 별칭 띄어쓰기 X, 특수문자 X, [문자]만 가능
 * 
 * 컬럼명 AS "별칭" : 별칭 띄어쓰기, 특수문자, 문자, 전부가능
 *  
 * AS는 생략 가능
 * 
 */
...
-- DISTINCT : 조회 시 컬럼에 포함된 중복 값을 한번 만 표기
-- 주의사항 1) DISTINCT 구문은 SELECT 마다 딱 한번 씩만 작성 가능
-- 주의사항 1) DISTINCT 구문은 SELECT 바로 다음 작성 되어야 한다.
...
FROM EMPLOYEE
WHERE DEPT_CODE = 'D9'; -- 비교 연산자 (=) / 대입연산자는 (:=)
...
-- NOT BETWEEN A AND B : A이상 B이하가 아닌경우
						> A미만, B초과 
...
-- 여러 조건을 쓸땐 JAVA처럼 ' ,  ' 가 아니라 AND로 조건 추가
...
/* NOT IN */
-- EMPLOYEE 테이블에서
-- 부서코드가 D1, D6, D9이 [아닌 사원]의
-- 사번, 이름, 부서코드 조회
SELECT  EMP_ID , EMP_NAME , DEPT_CODE 
FROM EMPLOYEE
WHERE DEPT_CODE NOT IN('D1','D6','D9') -- 12명 
OR DEPT_CODE IS NULL; -- 부서 코드가 없는 2명 ( NULL 처리 연산자 'IS NULL')
...
** SELECT문 해석 시 가장 마지막에 해석 된다!!! 중요! 항상 제일 마지막

3. SELECT 절
1. FROM 절 
2 .WHERE 절
4. ORDER BY 컬럼명 | 별칭 | 컬럼 순서 [ASC | DESC] (어센딩 디센딩) | [NULLS FIRST|LAST]
															(오름 / 내림 차순) 대괄호애들은 생략가능.
...
SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
ORDER BY SALARY; -- ASC가 기본값이다. 안쓰면 기본으로 오름차순
