...
-- 사용자 계정 생성
CREATE USER member_bsh IDENTIFIED BY member1234;

-- 생성한 사용자 계정에 권한 부여 
GRANT CONNECT, RESOURCE, CREATE VIEW TO member_bsh;

-- 테이블 스페이스 할당
ALTER USER member_bsh DEFAULT 
...
CREATE TABLE "BOARD" (
			...
			BOARD_TITLE VARCHAR2 (500) NOT NULL, -- 제목 꼭 있어야하니까 NOT NULL 제약조건
...
COMMENT ON COLUMN "BOARD".DELETE_FL 				  IS '삭제 여부';
...
-- 댓글 테이블 생성 ( COMMENT = 댓글(대꾸하는 글),덧글(덧붙히는 글) / REPLY = 답글 )
CREATE TABLE "COMMENT"(
			COMMENT_NO NUMBER,
			COMMENT_CONTENT VARCHAR2(900) NOT NULL,
			CREATE_DT DATE DEFAULT SYSDATE,
			DELETE_FL CHAR(1) DEFAULT 'N' CHECK(DELETE_FL IN('N' , 'Y')),
			MEMBER_NO NUMBERREFERENCES "MEMBER",
			BOARD_NO REFERENCES "BOARD",
...
--  게시글 목록 조회 (댓글 수 포함 -> 상관쿼리)   CREATE_DT = 작성일 / READ_COUNT = 조회수
SELECT BOARD_NO, BOARD_TITLE, MEMBER_NM, READ_COUNT, 
--
			CASE 				-- 현재시간   -  게시글 작성일	<  분 단위
...
-- 1일 11시간  = 1.47 ( 1 = 하루 / 47은 47%정도 = 하루 절반정도 지났따.)
...
-- 특정 게시글의 댓글 목록 조회(작성일 오름차순(DEFAULT로 ASC) = NOT DESC)
