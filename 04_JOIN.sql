...
-- 컬럼 수를 기준으로 JOIN
--> 왼편에 작성된 테이블의 모든 행이 결과에 포함 되어야 한다. (JOIN이 안되는 행도 결과 포함 = NULL)
-- ANSI 표준
SELECT EMP_NAME, DEPT_TITLE
...
WHERE DEPT_CODE = DEPT_ID(+); -- 오라클에서 LEFT JOIN을 쓰려면 WHERE절의 반대쪽 테이블 컬럼에 (+)를 해야 한다. (이상해..)
...
ORDER BY 2; -- 숫자는 정렬하고 싶은 SELECT절 목록순서
...
-- 4. 비등가 조인(NON EQUAL JOIN) 등호가 아닌, BETWEEN AND 같은 구문을 사용

-- '='(등호)를 사용하지 않는 조인문
--  지정한 컬럼 값이 일치하는 경우가 아닌, 값의 범위에 포함되는 행들을 연결하는 방식

SELECT * FROM SAL_GRADE; -- SAL = SALARY

SELECT EMP_NAME, SAL_LEVEL FROM EMPLOYEE e ;

-- 사원의 급여에 따른 급여 등급 조회하기
...
FROM EMPLOYEE e 
-- JOIN JOB USING(JOB_CODE); -- USING은 두 테이블의 컬럼명이 같을때 
NATURAL JOIN JOB;

SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE e 
NATURAL JOIN DEPARTMENT; -- 잘못 조인되면 CROSS JOIN결과가 조회된다.
...
-- 사원이름, 부서명, 지역명 조회
--> 테이블은 MPLOYEE, DEPARTMENT, LOCATION 
-- ANSI 표준
...
--> 해결방법 : DEPARTMENT와 LPCATION조인 순서를 바꿔서
--						EMPLOYEE와 DEPARTMENT가 조인된 결과를 먼저 만들어
--						LOCATION_ID 컬럼이 존재할 수 있도록 만든다.                             <-- 문제해결 시험 모범 답안

-- 오라클 전용
SELECT EMP_NAME , DEPT_TITLE, LOCAL_NAME
FROM EMPLOYEE , DEPARTMENT d ,LOCATION l 
...
-- 3. 해외영업 1부(D5), 2부(D6)에 근무하는 사원의 
-- 사원명, 직급명, 부서코드, 부서명 조회하시오.
SELECT EMP_NAME , DEPT_CODE, JOB_CODE , JOB_NAME
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN JOB  USING(JOB_CODE)
