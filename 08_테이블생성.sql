...
*/
SELECT * FROM USER_TABLES; -- 사용자가 가지고 있는 테이블 조회
...
-- TCL ( Transaction Control Language) : 트랜잭션 제어 언어, DML수행 내용을 커밋/롤백하는 언어
...
-- 데이터 베이스 내에서 모든 [데이터]는 [테이블을 통해서 저장]된다.
...
CREATE TABLE TLM (
				TLM_ID VARCHAR2(200),
				TLM_NAME VARCHAR2(200),
				TLM_PRICE NUMBER);

SELECT TLM_ID 메뉴번호, TLM_NAME 메뉴이름, TLM_PRICE 가격 FROM TLM

COMMENT ON COLUMN TLM.TLM_ID IS '메뉴 번호';
COMMENT ON COLUMN TLM.TLM_NAME IS '메뉴 이름';
COMMENT ON COLUMN TLM. TLM_PRICE IS '메뉴 가격';

INSERT INTO TLM VALUES('KOR1','김밥','3000');
INSERT INTO TLM VALUES('KOR2','라면','4000');
INSERT INTO TLM VALUES('KOR3','','3000');
...
CREATE TABLE "MEMBER"(
			...
			MEMBER_SSN CHAR(14), -- 991213-1234567 주민번호는 고정이기 때문에 고정길이 문자열 사용
			ENROLL_DATE DATE DEFAULT SYSDATE-- 가입이 된 날짜 ( 기본적으로 현재 시간 )
);

-- SQL 작성법 : 대문자 작성 권장, 연결된 단어 사이에는 "_" (언더바) 사용 
-- 문자 인코딩 UTF-8 : 영어,숫자는 1BYTE, 한글,특수문자 등은 3BYTE 취급 
...
INSERT INTO MEMBER VALUES('MEM01','123ABC', '홍길동' , '991213-1234567'
															, DEFAULT); -- < DEF자리에 SYS
-- * INSERT/UPDATE 시 컬럼 값으로 DEFAUAT를 작성하면
--   테이블 생성 시 해당 컬럼에 지정된 DAFAULT 값으로 삽입이 된다!														
...
-- 추가 샘플 데이터 삽입
-- 가입일 -> SYSDATE를 활용
INSERT INTO MEMBER VALUES('MEM02','1234QWER', '백승훈' , '920808-1012345', SYSDATE); 

-- 가입일 -> DEFAULT 활용(테이블 생성 시 정의된 값이 반영됨)
INSERT INTO MEMBER VALUES('MEM03','Q1W2E3R', '김장훈' , '770708-1056789', DEFAULT);
...
VALUES('MEM04', '1A2A3A4A', '박희순');

-- ** JDBC에서  날짜를 입력 받았을 때 삽입하는 방법 **
-- EX) '2022-09-13 17:33:27'
INSERT INTO MEMBER VALUES('MEM05','PARK4042' ,'김득구', '881212-1578423',
...
INSERT INTO MEMBER2 VALUES('MEM01' , 'PAW0202', '고구마','77154215');
INSERT INTO MEMBER2 VALUES('MEM02' , '1P2O3KD', '정중앙','01048754877');
--> NUMBER 타입 컬럼에 데이터 삽입 시													--   1048754877
...
CREATE TABLE USER_USED_NN(
    USER_NO NUMBER NOT NULL,  -- 사용자 번호 컬럼 (모든 사용자는 사용자 번호가 있어야 한다)
    															--> 컬럼 레벨 제약 조건 설정
