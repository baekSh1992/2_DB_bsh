...
-- COMMIT(트랜잭션 종료 후 저장), ROLLBACK(트랜잭션 취소), SAVEPOINT(임시저장) 로 구성
-- 커밋 > 롤백 > 세이브포인트 로 구성
...
/* TRANSACTION이란?
 ...
 EX) INSERT 수행 --------------------------------> DB 반영(X)
    ...
    2) ROLLBACK : 메모리 버퍼(트랜잭션)에 임시 저장된 데이터 변경 사항을 삭제하고
                 마지막 COMMIT 상태로 돌아감.(DB에 변경 내용 반영 하지 않음)
...
ROLLBACK; -- ROLL백은 트랜잭션에만 영향 끼침 DB에는 영향X
...
-- 롤백 수행
ROLLBACK;
SELECT *FROM DEPARTMENT2; -- 개발 4팀 롤백 확인
